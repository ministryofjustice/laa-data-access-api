...
# Set the deployment host, this will add the prefix of the branch name e.g el-257-deploy-with-circleci or just main
  RELEASE_HOST="$BRANCH_RELEASE_NAME-laa-data-access-api-$ENVIRONMENT.cloud-platform.service.justice.gov.uk"
...
# Set the ingress name, needs release name, namespace and -green suffix
  ...
  helm upgrade "$BRANCH_RELEASE_NAME" .helm/data-access-api/. \
                ...
                --set env.DATA_STORE_BASE_URL="$DATA_STORE_BASE_URL" \
...
if [[ ("$ENVIRONMENT" == 'uat') && "$GITHUB_REF_NAME" == "main" ]] || \
   [[ (("$ENVIRONMENT" == 'staging' || "$ENVIRONMENT" == 'production') && "$GITHUB_REF_NAME" =~ $releaseTag) ]]
then
  echo "Deploying Main"
  deploy_main
else
