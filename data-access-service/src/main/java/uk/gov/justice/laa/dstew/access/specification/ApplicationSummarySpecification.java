...
public class ApplicationSummarySpecification {
  ...
  public static Specification<ApplicationEntity> filterBy(
    ...
    return isStatus(status)
        ...
        .and(filterIndividual(firstName, lastName, clientDateOfBirth))
  ...
  private static Specification<ApplicationEntity> filterIndividual(String firstName, String lastName, LocalDate dateOfBirth) {
    if (!isPopulated(firstName) && !isPopulated(lastName) && dateOfBirth == null) {
      return Specification.unrestricted();
    }

    return (root, query, builder) -> {
      Join<ApplicationEntity, IndividualEntity> join = root.join("individuals", JoinType.INNER);

      List<Predicate> predicates = new ArrayList<>();
      predicates.add(builder.equal(join.get("type"), IndividualType.CLIENT));
      if (isPopulated(firstName)) {
        predicates.add(builder.like(builder.lower(join.get("firstName")), "%" + firstName.toLowerCase() + "%"));
      }
      if (isPopulated(lastName)) {
        predicates.add(builder.like(builder.lower(join.get("lastName")), "%" + lastName.toLowerCase() + "%"));
      }
      if (dateOfBirth != null) {
        predicates.add(builder.equal(join.get("dateOfBirth"), dateOfBirth));
      }
