...
public interface ApplicationRepository
  ...
  @Query(
      value = "SELECT DISTINCT lead_application_id "
          + "FROM linked_applications "
          + "WHERE lead_application_id IN :pageIds OR associated_application_id IN :pageIds",
      nativeQuery = true)
  List<UUID> findAllLeadIdsByPageIds(@Param("pageIds") List<UUID> pageIds);
