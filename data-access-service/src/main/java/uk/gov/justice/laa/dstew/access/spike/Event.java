...
public record Event(
  ...
  public static ApplicationDomainEventResponse fromEvent(Event event) {
    ApplicationDomainEventResponse domainEvent = new ApplicationDomainEventResponse();
  ...
  public static Event convertToEvent(DomainEventEntity domainEventEntity) {
    return Event.builder()
        ...
        .caseworkerId(
            domainEventEntity.getCaseworkerId() != null
                ? String.valueOf(domainEventEntity.getCaseworkerId())
                : null)
        .timestamp(domainEventEntity.getCreatedAt())
        .description(
            domainEventEntity.getType().name()
                + " event for application "
                + domainEventEntity.getApplicationId())
        .requestPayload(domainEventEntity.getData())
        .domainEventId(
            domainEventEntity.getId() != null
                ? domainEventEntity.getId()
