...
public class SdsService {
  ...
  public DocumentUploadResponse saveFile(UUID applicationId, MultipartFile file) {
    ...
    builder.part(
        "body", String.format("{\"bucketName\":\"%s\",\"folder\":\"%s\"}", bucketName, folderName));

    DocumentUploadResponse sdsSaveFileResponse =
        sdsRestClient
            .post()
  ...
  public DocumentUpdateResponse saveOrUpdateFile(MultipartFile file) {
    ...
    DocumentUpdateResponse sdsSaveFileResponse =
        sdsRestClient
            .put()
  ...
  public DocumentDownloadResponse getFile(UUID applicationId, String documentId) {
    ...
    DocumentDownloadResponse sdsGetFileResponse =
        sdsRestClient
            .get()
  ...
  /** Delete files from the SDS service. @Param fileIds the list of file IDs to be deleted */
  public void deleteFiles(UUID applicationId, List<String> fileIds) {
    String folderName = applicationId.toString();
    List<String> fileKeys = fileIds.stream().map(fileId -> folderName + "/" + fileId).toList();
    sdsRestClient
        .delete()
  ...
  public SdsHealthResponse checkHealth() {
    SdsHealthResponse sdsHealthResponse =
        sdsRestClient
            .get()
