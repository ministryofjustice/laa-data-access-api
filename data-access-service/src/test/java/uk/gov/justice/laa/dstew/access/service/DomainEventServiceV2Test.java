...
public class DomainEventServiceV2Test extends BaseServiceTest {
  ...
  class GetEvents {
    ...
    void givenExpectedDomainEvents_whenGetEvents_thenReturnDomainEventsInCreatedAtOrder() {
      ...
      when(domainEventRepository.findAll(any(Specification.class)))
          .thenReturn(expectedDomainEvents);

      // when
      List<ApplicationDomainEvent> actualDomainEvents =
    ...
    private void assertDomainEventsEqual(
        List<DomainEventEntity> expected, List<ApplicationDomainEvent> actual) {
