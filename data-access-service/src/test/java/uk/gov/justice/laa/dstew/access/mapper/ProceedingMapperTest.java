...
class ProceedingMapperTest extends BaseMapperTest {
    ...
    void givenNullProceeding_whenToProceedingEntity_thenReturnNull() {
        assertThat(proceedingMapper.toProceedingEntity(null, ApplicationEntity.builder().id(UUID.randomUUID()).build())).isNull();
    }

    @Test
    void givenProceedingAndApplicationId_whenToProceedingEntity_thenMapsFieldsCorrectly() {
        UUID applicationId = UUID.randomUUID();
        ApplicationEntity applicationEntity = ApplicationEntity.builder().id(applicationId).build();
        Proceeding proceeding = DataGenerator.createDefault(ProceedingGenerator.class);

        ProceedingEntity result = proceedingMapper.toProceedingEntity(proceeding, applicationEntity);

        assertThat(result.getApplyProceedingId()).isEqualTo(proceeding.getId());
        assertThat(result.getApplication().getId()).isEqualTo(applicationId);
    ...
    void givenProceedingWithAllNullFields_whenToProceedingEntity_thenNullableFieldsAreNull() {
        ...
        ProceedingEntity result = proceedingMapper.toProceedingEntity(proceeding, (ApplicationEntity) null);
    ...
    @Test
    void givenProceedingEntityWithScopeLimitations_whenToApplicationProceeding_thenMapsScopeLimitationsCorrectly() {
        ProceedingEntity entity = DataGenerator.createDefault(ProceedingsEntityGenerator.class);
    ...
    @Test
    void givenProceedingEntityWithScopeLimitationsMissingDescription_whenToApplicationProceeding_thenDescriptionIsNull() {
        ProceedingEntity entity = DataGenerator.createDefault(ProceedingsEntityGenerator.class,
                builder -> builder.proceedingContent(Map.of(
                        "meaning", "hearing",
    ...
    @Test
    void givenProceedingEntityWithScopeLimitationsMissingMeaning_whenToApplicationProceeding_thenScopeLimitationIsNull() {
        ProceedingEntity entity = DataGenerator.createDefault(ProceedingsEntityGenerator.class,
                builder -> builder.proceedingContent(Map.of(
                        "meaning", "hearing",
                        "scopeLimitations", List.of(
