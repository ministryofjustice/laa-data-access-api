...
public class TestHarness {
  ...
  private static final ApplicationUpdateRequestFactory applicationUpdateRequestFactory =
      new ApplicationUpdateRequestFactory();
  private static final ApplicationCreateRequestFactory applicationCreateRequestFactory =
      new ApplicationCreateRequestFactory(
          new IndividualFactory(), new RequestApplicationContentFactory());
  ...
  public static void main(String[] args) throws Exception {
    ...
    for (int i = 0; i < numberIdsToGenerate; i++) {
      ...
      String newId =
          "LAA-" + UUID.randomUUID().toString().replace("-", "").substring(0, 8).toUpperCase();
      ids.add(newId);
    }
    ...
    List<ApplicationCreateRequest> createRequests =
        ids.stream()
            .parallel()
            .map(
                id ->
    ...
    List<PostResult> postResults =
        createRequests.stream()
            ...
            .filter(
                result ->
                    result != null && result.location != null && result.location.contains("/"))
            .toList();
    ...
    System.out.println(
        "Total time: "
            + postDuration
            + " ms ("
            + String.format("%.2f", postDuration / 1000.0)
            + " seconds)");
    if (!postResults.isEmpty()) {
      ...
      System.out.println(
          "Throughput: " + String.format("%.0f", throughputPerSecond) + " requests/second");
    }
    System.out.println();

    // Create a list of update requests with their patch URLs
    List<UpdateRequestData> updateRequests =
        postResults.stream()
            .parallel()
            .map(
                result -> {
                  ...
                  try {
                    UUID.fromString(appId); // Validate UUID
                  } catch (Exception e) {
                    System.out.println(
                        "Invalid UUID in Location header, skipping PATCH for id: " + appId);
                    return null;
                  }
                  ...
                  return new UpdateRequestData(patchUrl, updateRequest);
                })
    ...
    System.out.println(
        "Total time: "
            + patchDuration
            + " ms ("
            + String.format("%.2f", patchDuration / 1000.0)
            + " seconds)");
    if (!updateRequests.isEmpty()) {
      ...
      System.out.println(
          "Average time per PATCH: " + String.format("%.2f", avgTimePerPatch) + " ms");
      System.out.println(
          "Throughput: " + String.format("%.0f", throughputPerSecond) + " requests/second");
    }
    System.out.println();
    ...
    if (totalOperations > 0) {
      ...
      System.out.println(
          "Overall throughput: " + String.format("%.0f", overallThroughput) + " requests/second");
    }
  }

  private static @NonNull HttpResponse<String> getPostResponse(
      ApplicationCreateRequest createRequest, HttpClient client)
    ...
    //    System.out.println("POST " + postUrl + " => " + postResponse.statusCode());
  ...
  private static void performPatchRequests(
      List<UpdateRequestData> updateRequests, HttpClient client) {
    updateRequests.stream()
        ...
        .forEach(
            data -> {
              try {
                String patchBody =
                    MapperUtil.getObjectMapper().writeValueAsString(data.updateRequest);
                ...
                //        System.out.println("PATCH " + data.patchUrl + " => " +
  ...
  private static @Nullable List<String> getIds() {
    ...
    try (InputStream is = TestHarness.class.getResourceAsStream(IDS_FILE)) {
      ...
      try (BufferedReader reader = new BufferedReader(new InputStreamReader(is))) {
        ids = reader.lines().map(String::trim).filter(line -> !line.isEmpty()).toList();
      }
    } catch (Exception e) {
