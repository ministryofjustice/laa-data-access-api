...
public class HarnessExtension implements BeforeAllCallback, AfterAllCallback, TestInstancePostProcessor {
    ...
    public void beforeAll(ExtensionContext ctx) {
        ...
        if (HarnessMode.isInfrastructure()) {
            ...
            store.getOrComputeIfAbsent(ROW_COUNT_SNAPSHOT_KEY + ".asserter",
                    key -> {
                        ...
                        return (ExtensionContext.Store.CloseableResource) () -> {
                            log.info("[HarnessExtension] End-of-suite row-count parity asserter closing — running assertion");
                            @SuppressWarnings("unchecked")
                            var snapshot = (java.util.Map<String, Long>) store.get(ROW_COUNT_SNAPSHOT_KEY);
                            if (snapshot != null) {
                                provider.getBean(TableRowCountAssertion.class)
                                        .assertRowCountsMatch(snapshot, "full-infrastructure-suite");
                                log.info("[HarnessExtension] Row-count parity assertion PASSED");
                            } else {
                                log.warn("[HarnessExtension] Row-count snapshot was null — parity assertion skipped");
                            }
                        };
                    });
        }
