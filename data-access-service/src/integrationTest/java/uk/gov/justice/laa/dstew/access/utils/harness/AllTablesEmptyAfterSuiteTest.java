...
/**
 * End-of-suite sentinel: verifies that every application-domain table is empty
 * after the complete integration-test suite has run.
 *
 * <p>This test is placed last in execution order (via {@link Order @Order(Integer.MAX_VALUE)})
 * so it fires after all other integration tests and their teardowns have completed.
 *
 * <p>It is intentionally minimal — the heavy lifting is done by the
 * {@link DatabaseCleanlinessAssertion} that is already wired into every
 * {@code @AfterEach} in {@link BaseHarnessTest}.  This class provides an explicit,
 * named failure point in the test report if any data leaks across the entire suite.
 */
