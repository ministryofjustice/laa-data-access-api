...
public class GetCaseworkersTest extends BaseHarnessTest {
  ...
  void givenRoleReaderAndIncorrectHeader_whenGetCaseworkers_thenReturnBadRequest(String serviceName)
      throws Exception {
  ...
  private void verifyServiceNameHeader(String serviceName) throws Exception {
    HarnessResult result =
        getUri(TestConstants.URIs.GET_CASEWORKERS, ServiceNameHeader(serviceName));
  ...
  public void givenRoleReader_whenGetCaseworkers_thenReturnOk() throws Exception {
    ...
    List<CaseworkerResponse> actualCaseworkerResponses =
        objectMapper.readValue(
            result.getResponse().getContentAsString(),
            objectMapper
                .getTypeFactory()
                .constructCollectionType(List.class, CaseworkerResponse.class));
  ...
  private void assertCaseworkerListEquals(
      List<CaseworkerResponse> caseworkers, List<CaseworkerEntity> entities) {
    for (CaseworkerEntity entity : entities) {
      assertThat(caseworkers)
          .anyMatch(
              c ->
                  entity.getId().equals(c.getId()) && entity.getUsername().equals(c.getUsername()));
    }
  }
}
