...
/**
 * Proves that the harness teardown (tearDownTrackedData) only removes rows whose IDs were
 * registered automatically via PersistedDataGenerator, and never touches data that was persisted
 * outside that mechanism (e.g. directly via a repository).
 *
 * <p>Tests are NOT annotated @SmokeTest — they run only in integration mode (Testcontainers)
 * against an empty database. They must not run against real infrastructure because the sentinel
 * data created here has no meaning there.
 *
 * <p>Sentinel entities are created in @BeforeAll / cleaned in @AfterAll so they exist across all
 * test methods but are never registered with the harness tracker, simulating data that existed
 * before the test run.
 */
...
public class HarnessDataIsolationTest extends BaseHarnessTest {
  ...
  /**
   * Sentinel rows are intentionally alive across all test methods in this class — the whole point
   * of these tests is to prove the harness never deletes pre-existing data. Suppress the per-test
   * cleanliness assertion inherited from BaseHarnessTest so it does not fail while sentinels are
   * still present.
   *
  ...
  /**
   * Creates sentinel rows directly via the repositories, bypassing PersistedDataGenerator, so they
   * represent pre-existing data the harness must never delete.
   */
  @BeforeAll
  void createSentinels() {
    var cwRepo =
        harnessProvider.getBean(
            uk.gov.justice.laa.dstew.access.repository.CaseworkerRepository.class);
    var appRepo =
        harnessProvider.getBean(
            uk.gov.justice.laa.dstew.access.repository.ApplicationRepository.class);
    var deRepo =
        harnessProvider.getBean(
            uk.gov.justice.laa.dstew.access.repository.DomainEventRepository.class);

    CaseworkerEntity sentinel =
        DataGenerator.createDefault(
            CaseworkerGenerator.class, b -> b.username("SENTINEL_PRE_EXISTING").build());
    cwRepo.saveAndFlush(sentinel);
    sentinelCaseworkerId = sentinel.getId();

    ApplicationEntity sentinelApp =
        DataGenerator.createDefault(
            ApplicationEntityGenerator.class, b -> b.caseworker(sentinel).build());
    appRepo.saveAndFlush(sentinelApp);
    sentinelApplicationId = sentinelApp.getId();

    DomainEventEntity sentinelDe =
        DataGenerator.createDefault(
            DomainEventGenerator.class,
            b ->
                b.applicationId(sentinelApp.getId())
  ...
  /**
   * Asserts that every sentinel still exists after all tests and their @AfterEach teardowns have
   * run, then removes them. This is the definitive proof that the harness never deletes
   * pre-existing data.
   */
  @AfterAll
  ...
  /**
   * Deletes an application (looked up by ID) together with every individual row that was
   * cascade-persisted alongside it.
   *
   * <p>Deleting an application CASCADE-deletes the {@code linked_individuals} join rows but NOT the
   * {@code individuals} rows themselves (they are independent entities). This helper fetches the
   * linked individual IDs via JDBC before deleting the application, then removes those individuals
   * afterwards — exactly mirroring what {@link
   * uk.gov.justice.laa.dstew.access.utils.generator.PersistedDataGenerator#deleteTrackedData()}
   * does for tracked applications.
   *
   * <p>Use this anywhere an application was saved directly via a repository (bypassing {@code
   * PersistedDataGenerator}) and must be manually cleaned up.
   */
  private void deleteWithLinkedIndividuals(UUID applicationId) {
    ...
    var indivRepo =
        harnessProvider.getBean(
            uk.gov.justice.laa.dstew.access.repository.IndividualRepository.class);
  ...
  /**
   * Proves that the sentinel caseworker (not tracked) is still present after @BeforeEach runs, and
   * that the harness-seeded caseworkers exist during the test body. After this method
   * returns, @AfterEach will remove JohnDoe and JaneDoe (they were auto-tracked) but leave the
   * sentinel alone.
   */
  @Test
  ...
  /**
   * Proves that the sentinel application (not tracked) survives teardown while a test-created
   * application (auto-tracked via persistedDataGenerator) is removed.
   */
  @Test
  void givenPreExistingApplication_whenHarnessTeardownRuns_thenOnlyTrackedApplicationIsRemoved() {
    ...
    ApplicationEntity testApp =
        persistedDataGenerator.createAndPersist(
            ApplicationEntityGenerator.class, b -> b.caseworker(CaseworkerJohnDoe).build());
  ...
  /**
   * Proves that the sentinel domain event (not tracked) survives teardown while a test-created
   * domain event (auto-tracked) is removed.
   */
  @Test
  void givenPreExistingDomainEvent_whenHarnessTeardownRuns_thenOnlyTrackedDomainEventIsRemoved() {
    ...
    DomainEventEntity testDe =
        persistedDataGenerator.createAndPersist(
            DomainEventGenerator.class,
            b ->
                b.applicationId(sentinelApplicationId)
   ...
   * persistedDataGenerator) is NOT tracked and therefore survives teardown; one persisted via
   * persistedDataGenerator IS tracked and IS removed.
   */
  @Test
  void givenUntrackedAndTrackedEntities_whenHarnessTeardownRuns_thenOnlyTrackedEntityIsRemoved() {
    ...
    ApplicationEntity entityA =
        DataGenerator.createDefault(
            ApplicationEntityGenerator.class, b -> b.caseworker(CaseworkerJohnDoe).build());
    applicationRepository.saveAndFlush(entityA);

    // entityB — persisted via persistedDataGenerator, auto-tracked
    ApplicationEntity entityB =
        persistedDataGenerator.createAndPersist(
            ApplicationEntityGenerator.class, b -> b.caseworker(CaseworkerJaneDoe).build());
    ...
    try {
      ...
      applicationRepository.findById(entityA.getId()).ifPresent(applicationRepository::delete);
    }
  }
  ...
  /**
   * Proves that if setup partially fails, any entity that was already persisted (and therefore
   * auto-tracked via PersistedDataGenerator) is still cleaned up by teardown, even though setup did
   * not complete.
   *
   * <p>Simulated manually so the result is visible within this test rather than relying on
   * post-test @AfterEach only.
   */
  @Test
  void givenPartialSetupFailure_whenTeardownRuns_thenPartiallyCreatedDataIsRemoved() {
    ...
    CaseworkerEntity partialCaseworker =
        persistedDataGenerator.createAndPersist(
            CaseworkerGenerator.class, b -> b.username("PARTIAL_SETUP_CW").build());
