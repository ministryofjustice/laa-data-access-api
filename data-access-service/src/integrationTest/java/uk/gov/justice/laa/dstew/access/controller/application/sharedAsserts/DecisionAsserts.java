 ...
 *
 * <p>Methods are {@link Transactional} so that the JPA session remains open while
 * lazy collections (e.g. {@code DecisionEntity.meritsDecisions}) are traversed,
...
public class DecisionAsserts {
    ...
    /**
     * Fetches the saved {@link DecisionEntity} for the given application and asserts that it
     * matches the expected {@link MakeDecisionRequest}.
     *
     * <p>All repository and collection access happens within a single transaction so lazy
     * collections are initialised before the session closes.
     */
    @Transactional
    public void verifyDecisionSavedCorrectly(UUID applicationId, MakeDecisionRequest expectedMakeDecisionRequest) {
