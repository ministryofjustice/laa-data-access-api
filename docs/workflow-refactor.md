...
Before                              After
──────────────────────────────────  ──────────────────────────────────────────────────
build-test-deploy.yml               build-and-test-pr.yml   ← manual (workflow_dispatch)
(one file, all events)              deploy-ephemeral-pr.yml ← deploys PR preview envs
                                    deploy-main.yml         ← triggered on push to main
```
...
Runs the full build-and-test pipeline on demand (manual workflow), useful for ad-hoc validation without deployment.
...
### `deploy-ephemeral-pr.yml`
**Trigger:** `pull_request` (opened, synchronize, reopened) or `workflow_dispatch`

Builds, scans, pushes Docker images, deploys an ephemeral PostgreSQL-backed preview environment in UAT, and runs smoke tests.

| Job | Depends on | What it does |
|-----|-----------|--------------|
| `build-test` | — | Gradle build + unit/integration tests, publishes API models package as a `SNAPSHOT`, uploads JARs as artifacts |
| `vulnerability-scan-app` | `build-test` | Snyk code scan |
| `vulnerability-scan-docker` | `build-test` | Snyk Docker scan |
| `build-push-docker` | `build-test`, both Snyk jobs | Builds & pushes main service image |
| `build-push-mass-generator-docker` | `build-test`, both Snyk jobs | Builds & pushes mass-generator image |
| `deploy-ephemeral` | both push jobs + scans | Deploys ephemeral PostgreSQL + app release via Helm |
| `uat-smoke-test` | `deploy-ephemeral` | Runs smoke tests against the ephemeral release |
...
───────────────────────────────────────────────────────────────────────────
PR opened/updated         deploy-ephemeral-pr.yml     deploy-ephemeral-release (ephemeral DB)
